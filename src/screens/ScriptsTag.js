...
class ScriptsTags extends Component {
  ...
  componentDidMount() {
    ...
    if (type === 'hashtags') {
      axios
        .get(`${apiurl}fetch-tag-scripts.php?id=${id}&user_id=${user_id}`)
        .then(res => {
      ...
      axios
        .get(`${apiurl}fetch-place-scripts.php?place=${address}&user_id=${user_id}`)
        .then(res => {
          console.log(res.data);
          this.setState({ data: res.data });
        })
...
export default connect(mapStateToProps)(ScriptsTags);
