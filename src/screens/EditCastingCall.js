...
class EditCastingCall extends Component {
    ...
    componentDidMount() {
        ...
        const { role_details } = casting_call_details;
        const call_details = casting_call_details.data[0];
        const { city, country, formatted_address } = call_details;;
        const lng = call_details.longitude;
        const lat = call_details.latitude;

        const location = { city, country, formatted_address, lng, lat };
        this.props.addlocation(location);
        this.setState({
            startDate: call_details.start_date,
            endDate: call_details.deadline,
            description: call_details.description,
            date_venue: call_details.date_venue,
            title: call_details.title
        });
        console.log(casting_call_details);
    ...
    submitJob() {
        ...
        axios
            .post(`${apiurl}edit-casting-call.php?user_id=3`, data)
            .then(res => {
    ...
    render() {
        const { navigation, individualroles, selectedlocation } = this.props;
            ...
            <ScrollView contentContainerStyle={{ backgroundColor: '#f1f1f1' }}>
                ...
                <ScrollView
                    ...
                    <View style={{ backgroundColor: '#ffffff' }}>
                        ...
                        {role_details.map(e => (
                            <CastingCallRoles key={e.role_id} />
                        ))}
                    </View>
                </ScrollView>
                <View
...
export default connect(
    mapStateToProps,
    { rolesAction, submitcastingcalls, addlocation }
)(EditCastingCall);
